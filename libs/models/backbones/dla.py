...
class Channel_Att(nn.Module):
    def __init__(self, channels):
        ...
        # InstanceNorm 对于每个样本的每个通道进行归一化
        self.instance_norm = nn.InstanceNorm2d(self.channels, affine=True)  # affine=True 表示有可学习的缩放和平移参数

    def forward(self, x):
        residual = x
        
        # 对每个样本的每个通道进行归一化
        x = self.instance_norm(x)  # Shape: [B, C, H, W]
        
        # 根据 InstanceNorm 权重进行加权
        weight_in = self.instance_norm.weight.data.abs() / torch.sum(self.instance_norm.weight.data.abs())
        
        # 对每个通道进行加权
        x = x.permute(0, 2, 3, 1).contiguous()  # 转换为 [B, H, W, C]
        x = torch.mul(weight_in, x)  # 按照权重加权
