...
class MaxValueFusionECA(nn.Module):
    ...
    def forward(self, x):
        ...
        # Reshape to [B, C, n, 1, 1] so we can take the avg for each channel across different scales
        reshaped_output = concatenated_output.view(concatenated_output.size(0), -1, len(self.convs), 1, 1)  # [B, C, n, 1, 1]

        # Average value fusion: Take the average for each channel across different scales
        avg_value_fusion = torch.mean(reshaped_output, dim=2, keepdim=True)  # Shape: [B, C, 1, 1]

        # Apply sigmoid and return the final output
        attention = self.sigmoid(avg_value_fusion)  # Shape: [B, C, 1, 1]
