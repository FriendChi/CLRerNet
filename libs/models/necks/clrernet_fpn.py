...
class LGAG(nn.Module):
    ...
    def forward(self, g, x):
        ...
        fused = self.w_g * g1 + self.w_x * x1
        fused_norm = fused / (torch.norm(fused, p=2, dim=1, keepdim=True) + self.epsilon)

        psi = self.activation(fused_norm)
        psi = self.psi(psi)
...
class CLRerNetFPN(nn.Module):
    def __init__(self, in_channels, out_channels, num_outs):
        ...
        Args:
            in_channels (List[int]): Channel number list. 输入的特征图通道数列表。[128, 256, 512]
            out_channels (int): Number of output feature map channels. 输出特征图的通道数。64
